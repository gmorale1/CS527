...
class CASimulator():
    ...
    def simulate(self):
        ...
        for experiment in range(self.num_experiments):
            ...
            for z in range(len(self.rule_table)):
                ...
                zeta = self.calculate_Z(zeta)

                f.write(str(z) + "," + entry_zeroed + ",," + str(lam) + "," + str(lam_T) + "," + str(H) + "," + str(H_T) + "," + str(zeta) + ",\n")
    ...
    #Calculates the percent of the board that was in state 0
    def calculate_Z(self,zeta):
        return 1 - (zeta/(self.timesteps * self.dim)) 
